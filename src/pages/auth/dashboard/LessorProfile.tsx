...
const LessorProfile: React.FC = () => {
  const { t } = useTranslation()

  const [prenom, setPrenom] = useState('Ahmed')
  ...
  const [dateNaissance, setDateNaissance] = useState('15/03/1980')
      ...
      >
        <div className="flex flex-col gap-4">
          <Checkbox
            ...
            label="Newsletter (actualités et offres exclusives)"
          />
          <Checkbox
            checked={alertesEmail}
            onChange={() => setAlertesEmail(!alertesEmail)}
            label="Alertes Email (nouveaux locataires intéressés)"
          />
          <Checkbox
            checked={notificationsSms}
            onChange={() => setNotificationsSms(!notificationsSms)}
            label="Notifications SMS (alerte pour les demandes de location)"
          />
          <Checkbox
            checked={rapportMensuel}
            onChange={() => setRapportMensuel(!rapportMensuel)}
            label="Rapport mensuel de gestion (statistiques)"
          />
        </div>
      </SectionCard>
