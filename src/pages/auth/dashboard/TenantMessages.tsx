    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          {messages.filter(m => m.unread).length} message{messages.filter(m => m.unread).length > 1 ? 's' : ''} non lu{messages.filter(m => m.unread).length > 1 ? 's' : ''}
        </p>
      </div>
      ...
      <div className="bg-white border border-gray-200">
        <div className="px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <select className="border border-gray-300 px-3 py-2.5 text-sm focus:outline-none focus:border-emerald-600 bg-white">
              <option>Tous les dossiers</option>
              <option>Boîte de réception</option>
