...
import { buyerDocuments } from './data';

const BuyerDocuments: React.FC = () => {
  const docStats = [
    { label: 'Vérifiés', value: buyerDocuments.filter(d => d.statusType === 'verified' || d.statusType === 'signed').length, icon: CheckCircleIcon, color: 'text-emerald-600 bg-emerald-50 border-emerald-200' },
    { label: 'En attente', value: buyerDocuments.filter(d => d.statusType === 'pending').length, icon: ClockIcon, color: 'text-amber-600 bg-amber-50 border-amber-200' },
    { label: 'Manquants', value: buyerDocuments.filter(d => d.statusType === 'missing').length, icon: XCircleIcon, color: 'text-red-600 bg-red-50 border-red-200' },
  ];
